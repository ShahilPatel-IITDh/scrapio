	...
	$J.get(
		...
		switch( data.success )
		{
			case 1:
			{
				if ( callbackFunc )
				{
					var dialog = ShowConfirmDialog('Contenu non approprié pour tous les âges', data['html'], 'Voir le contenu' );
				...
				{
					ShowAlertDialog('Contenu non approprié pour tous les âges', data['html'] );
				}
			}
			break;
...
function UGCAdultContentPreferencesMenu( elSource )
{
	...
	// warning
	{
		...
		var $elViewWarning = $J( '<div/>' ).click( fnViewWarning ).text( 'Voir l\'avertissement de contenu pour adultes' ).addClass( 'option' );
...
var CCommentThread = Class.create( {
	...
	MarkSpam: function( gidComment, authorAccountID, fnOnSuccess )
	{
		...
		var dialog = ShowConfirmDialog( 'Marquer comme indésirable', 'Cette action entrainera le bannissement de l\'auteur(e) de la communauté, ajoutera ce commentaire aux données d\'apprentissage pour que nous puissions appliquer automatiquement un signalement à ce type de commentaires et éliminera tous les commentaires de l\'auteur(e). Voulez-vous vraiment continuer ?', 'Marquer comme indésirable' );
		dialog.done( function() {
