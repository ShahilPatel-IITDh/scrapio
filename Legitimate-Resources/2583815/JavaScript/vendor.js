...
return jQuery;}));;jQuery.fn.sortElements=(function(){var sort=[].sort;return function(comparator,getSortable){getSortable=getSortable||function(){return this};var placements=this.map(function(){var sortElement=getSortable.call(this),parentNode=sortElement.parentNode,nextSibling=parentNode.insertBefore(document.createTextNode(''),sortElement.nextSibling);return function(){if(parentNode===this){throw new Error("You can't sort elements if any one is a descendant of another.");}
parentNode.insertBefore(this,nextSibling);parentNode.removeChild(nextSibling);};});return sort.call(this,comparator).each(function(i){placements[i].call(getSortable.call(this));});};})();const rules={'auth.ajax_register_user':'/ajax/register-user','bitpay.ajax_bitpay_invoice':'/ajax/bitpay/invoice','checkout.ajax_checkout_with_bitpay':'/ajax/checkout/bitpay','checkout.ajax_checkout_with_cellsynt':'/ajax/checkout/cellsynt','checkout.ajax_checkout_with_perfectmoney':'/ajax/checkout/perfectmoney','checkout.ajax_checkout_with_redirect':'/ajax/checkout/<payment_system>','core.username_available':'/ajax/username/available','core_ajax.account_partial':'/ajax/block/account','core_ajax.usage_partial':'/ajax/block/usage','core_ajax.history_partial':'/ajax/block/history','core_ajax.ajax_resend_email':'/ajax/resend-email','core_ajax.cancel_subscription_billing':'/ajax/cancel_subscription_billing/<subscr_id>','core_ajax.changepwd':'/ajax/changepwd','core_ajax.changemail':'/ajax/changemail','core_ajax.user_flags':'/ajax/user-flags','core_ajax.bonus_email_invite':'/ajax/bonus/email-invite','stripe_ajax.ajax_stripe_subscr':'/ajax/stripe/subscr','stripe_ajax.ajax_stripe_charge':'/ajax/stripe/charge','core_ajax.port_forwarding':'/ajax/port-forwarding','core_ajax.wireguard':'/ajax/wireguard','core_ajax.wireguard_with_id':'/ajax/wireguard/<key_id>','core_ajax.get_regions':'/ajax/server-info/<protocol>'};url_for=function(endpoint,args){if(!(endpoint in rules)){throw 'No such endpoint: '+endpoint;}
var _url=rules[endpoint],_params={},_scheme,_external,_anchor;args=args||{};if(args._external==true){_external=true;_scheme=location.protocol.split(':')[0];delete args['_external'];}
if('_scheme'in args){if(_external){_scheme=args['_scheme'];delete args['_scheme'];}else{throw '_scheme is set without _external.';}}
if('_anchor'in args){_anchor=args['_anchor'];delete args['_anchor'];}
var _addTs=false;if('_addTs'in args){_addTs=args['_addTs'];delete args['_addTs'];}
for(var key in args){var value=args[key],r=new RegExp('<(.*:)?'+key+'>'),_tmp;_tmp=_url.replace(r,value);if(_url==_tmp){_params[key]=value;}else{_url=_tmp;}}
if(window.locale){_url='/'+window.locale+_url;}
_url=_url+(Object.keys(_params).length?('?'+$.param(_params)):'');if(_external){_url=_scheme+'://'+location.host+_url;}
if(_anchor){_url=_url+'#'+_anchor;}
if(_addTs){_url+='?ts='+(new Date().getTime());}
return _url;};function showError(message){$('.flash-message-container').html('');$("#messtemplate").tmpl({category:'error',message:message}).appendTo(".flash-message-container");$('html, body').animate({scrollTop:$('.page').offset().top},300);}
function showErrorResponse(generic_message){return function(response){let message;if(response.responseJSON&&response.responseJSON.reason){message=response.responseJSON.reason;}else{message=generic_message;}
showError(message);}};$(document).ready(function(){$('[data-toggle="tooltip"]').tooltip({placement:"auto right",container:"body",html:true});$('.auth-logo > i').click(function(e){e.stopPropagation();var dropdown=$('.dropdown');if(dropdown.is(':visible')){dropdown.hide();}else{dropdown.show();}});$('body').click(function(){$('.dropdown').hide();});$('.flash-message-container').on('click','.flash-message-close',function(){var msg=$(this).parent('.flash-message');msg.fadeOut('slow',function(){msg.remove();});});});/*!
...
var complete=function(){$('.navbar-collapse').removeClass('in');this.$element.removeClass('collapsing').addClass('collapse in')[dimension]('')
...
return false;}},{}],4:[function(require,module,exports){var bind=window.addEventListener?'addEventListener':'attachEvent',unbind=window.removeEventListener?'removeEventListener':'detachEvent',prefix=bind!=='addEventListener'?'on':'';exports.bind=function(el,type,fn,capture){el[bind](prefix+type,fn,capture||false);return fn;};exports.unbind=function(el,type,fn,capture){el[unbind](prefix+type,fn,capture||false);return fn;};},{}],5:[function(require,module,exports){function E(){}
E.prototype={on:function(name,callback,ctx){var e=this.e||(this.e={});(e[name]||(e[name]=[])).push({fn:callback,ctx:ctx});return this;},once:function(name,callback,ctx){var self=this;var fn=function(){self.off(name,fn);callback.apply(ctx,arguments);};return this.on(name,fn,ctx);},emit:function(name){var data=[].slice.call(arguments,1);var evtArr=((this.e||(this.e={}))[name]||[]).slice();var i=0;var len=evtArr.length;for(i;i<len;i++){evtArr[i].fn.apply(evtArr[i].ctx,data);}
return this;},off:function(name,callback){var e=this.e||(this.e={});var evts=e[name];var liveEvents=[];if(evts&&callback){for(var i=0,len=evts.length;i<len;i++){if(evts[i].fn!==callback)liveEvents.push(evts[i]);}}
(liveEvents.length)?e[name]=liveEvents:delete e[name];return this;}};module.exports=E;},{}],6:[function(require,module,exports){'use strict';exports.__esModule=true;var _createClass=(function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if('value'in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor);}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor;};})();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError('Cannot call a class as a function');}}
...
window.getSelection().removeAllRanges();};ClipboardAction.prototype.destroy=function destroy(){this.removeFake();};_createClass(ClipboardAction,[{key:'action',set:function set(){var action=arguments.length<=0||arguments[0]===undefined?'copy':arguments[0];this._action=action;if(this._action!=='copy'&&this._action!=='cut'){throw new Error('Invalid "action" value, use either "copy" or "cut"');}},get:function get(){return this._action;}},{key:'target',set:function set(target){if(target!==undefined){if(target&&typeof target==='object'&&target.nodeType===1){this._target=target;}else{throw new Error('Invalid "target" value, use a valid Element');}}},get:function get(){return this._target;}}]);return ClipboardAction;})();exports['default']=ClipboardAction;module.exports=exports['default'];},{}],7:[function(require,module,exports){'use strict';exports.__esModule=true;function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{'default':obj};}
function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError('Cannot call a class as a function');}}
function _inherits(subClass,superClass){if(typeof superClass!=='function'&&superClass!==null){throw new TypeError('Super expression must either be null or a function, not '+typeof superClass);}subClass.prototype=Object.create(superClass&&superClass.prototype,{constructor:{value:subClass,enumerable:false,writable:true,configurable:true}});if(superClass)Object.setPrototypeOf?Object.setPrototypeOf(subClass,superClass):subClass.__proto__=superClass;}
var _clipboardAction=require('./clipboard-action');var _clipboardAction2=_interopRequireDefault(_clipboardAction);var _delegateEvents=require('delegate-events');var _delegateEvents2=_interopRequireDefault(_delegateEvents);var _tinyEmitter=require('tiny-emitter');var _tinyEmitter2=_interopRequireDefault(_tinyEmitter);var Clipboard=(function(_Emitter){_inherits(Clipboard,_Emitter);function Clipboard(selector,options){_classCallCheck(this,Clipboard);_Emitter.call(this);this.resolveOptions(options);this.delegateClick(selector);}
Clipboard.prototype.resolveOptions=function resolveOptions(){var options=arguments.length<=0||arguments[0]===undefined?{}:arguments[0];this.action=typeof options.action==='function'?options.action:this.defaultAction;this.target=typeof options.target==='function'?options.target:this.defaultTarget;this.text=typeof options.text==='function'?options.text:this.defaultText;};Clipboard.prototype.delegateClick=function delegateClick(selector){var _this=this;this.binding=_delegateEvents2['default'].bind(document.body,selector,'click',function(e){return _this.onClick(e);});};Clipboard.prototype.undelegateClick=function undelegateClick(){_delegateEvents2['default'].unbind(document.body,'click',this.binding);};Clipboard.prototype.onClick=function onClick(e){if(this.clipboardAction){this.clipboardAction=null;}
this.clipboardAction=new _clipboardAction2['default']({action:this.action(e.delegateTarget),target:this.target(e.delegateTarget),text:this.text(e.delegateTarget),trigger:e.delegateTarget,emitter:this});};Clipboard.prototype.defaultAction=function defaultAction(trigger){return getAttributeValue('action',trigger);};Clipboard.prototype.defaultTarget=function defaultTarget(trigger){var selector=getAttributeValue('target',trigger);if(selector){return document.querySelector(selector);}};Clipboard.prototype.defaultText=function defaultText(trigger){return getAttributeValue('text',trigger);};Clipboard.prototype.destroy=function destroy(){this.undelegateClick();if(this.clipboardAction){this.clipboardAction.destroy();this.clipboardAction=null;}};return Clipboard;})(_tinyEmitter2['default']);function getAttributeValue(suffix,element){var attribute='data-clipboard-'+suffix;if(!element.hasAttribute(attribute)){return;}
return element.getAttribute(attribute);}
exports['default']=Clipboard;module.exports=exports['default'];},{"./clipboard-action":6,"delegate-events":1,"tiny-emitter":5}]},{},[7])(7)});