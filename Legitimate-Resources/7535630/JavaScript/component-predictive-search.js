...
Line 51:10 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
...
Line 66:9 Transforming object literal extensions to the configured target environment ("es5") is not supported yet
...
class PredictiveSearch extends HTMLElement {
  constructor() {
    ...
    this.searchPagesContainer = this.querySelector('[data-search-pages]');
    this.isOpen = false;
  ...
  getSearchResults(searchTerm) {
    ...
    const searchUrl = new URL(routes.predictive_search_url, window.top.location.origin)
    searchUrl.searchParams.set('q', searchTerm);
    searchUrl.searchParams.set('resources[type]', this.dataset.resources || 'product')
    searchUrl.searchParams.set('resources[limit]', this.dataset.limit || 4)
    searchUrl.searchParams.set('section_id', 'predictive-search')
    searchUrl.searchParams.set('resources[options][fields]', 'title,variants.title,tag,body,product_type,author,vendor,variants.sku')

    fetch(searchUrl)
      .then(async (response) => {
        if (!response.ok) {
          const message = await response.text();
          var error = new Error(response.status + ' ' + message);
          this.close();
          throw error;
        }
  ...
  open() {
    ...
    if (this.searchPagesContainer) {
      this.searchPagesContainer.dataset.activePage = 2;
    }
  }
  ...
  close(clearSearchTerm = false) {
    ...
    if (this.searchPagesContainer) {
      this.searchPagesContainer.dataset.activePage = 1;
    }
  }
}
