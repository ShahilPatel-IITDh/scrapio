...
tt_above = true,          // true if T_ABOVE cmd
...
function tt_Init()
{
	if (!(tt_op || tt_n4 || tt_n6 || tt_ie || tt_w3c) /*|| !(location.href.match(/http\:\/\/(www\.)?(walterzorn|devira)/))*/) return;
	...
	var i = tt_tags.length; while (i--)
	{
		...
		var j = tags.length; while (j--)
		{
			...
			{
				if (typeof (t_tj = tags[j]).onclick == 'function' && t_tj.onclick.toString().indexOf(esc) != -1 && !tt_n6 || tt_n6 && (over = t_tj.getAttribute('onclick')) && over.indexOf(esc) != -1)
				{
					if (over) t_tj.onclick = new Function(over);
					var txt = unescape(t_tj.onclick());
					htm += tt_Htm(
						t_tj,
						'tOoLtIp'+i+''+j,
						txt.wzReplace('& ','&')
