...
// Borrowed from PP.org; modified from http://justinhileman.info/article/a-jquery-widont-snippet/
(function( $ ) {

    "use strict";

    var widont = function( els ) {
        $( els ).each( function() {
            var $el = $( this );

            $el.html(
                $el.html().replace( /\s([^\s<]+)\s*$/,'&nbsp;$1' )
            );
        } );
    };
...
/*
“Sticky table headers” script

Courtesy of https://github.com/zachleat/BUSYNESS/blob/master/public/javascripts/lib/tables.stickyheaders.js
*/
;(function( win, $ ) {
    ...
    var S = {
        ...
        tests: {
            ...
            fixed: PP.utils.featureTest( 'position', 'fixed', true )
        },
        // Thanks jQuery!
        ...
        init: function( table ) {
            var $table = $( table ),
                ...
                updateCell = function( $new, $old ) {
                    // Adding +1px to each cell fixes an alignment bug in Chrome
                    $new.replaceWith( $old.clone().css( "width", ( $old[ 0 ].clientWidth + 1 ) + "px" ) );
                },
                updateClonedHeaders = function() {
                    var $clonedHeaders = $cloned.find( "th" );
                    // Let’s get the width of our “real” table
                    var originalTableStyle = ( window.getComputedStyle ) ? getComputedStyle( $table[ 0 ], null ) : $table[ 0 ].currentStyle;
                    var originalTableWidth = parseInt( originalTableStyle.getPropertyValue( "width" ), 10 );
                    // Non-Gecko browsers are naffing up the cell calculations, so let’s introduce a (ugh) UA-specific offset
                    var cellOffset = ( navigator.userAgent.toLowerCase().indexOf( "firefox" ) > -1 ) ? 0 : 1;

                    // Let’s apply the updated width to the cloned “sticky” header
                    $cloned.css( "width", ( originalTableWidth - cellOffset ) + "px" );
            ...
            // Update our “sticky” header on resize
            var resizeTimer;

            // Note: we have to debounce this handler slightly because of a race condition with tablesaw, which would sometimes “hide” our cells after our resize handler would fire.
...
(function( $, win, undefined ) {
    ...
    var componentName = "table--shortened",
        enhancedAttr = "data-enhanced-" + componentName,
        initSelector = "." + componentName + ":not([" + enhancedAttr + "])";

    // Classes!
    var parentClass = "container-shortened",
        excludeElements = ".stickyclone",
        topClass = "at-top",
        bottomClass = "at-bottom";

    $.fn[ componentName ] = function(){
        return this.not( excludeElements ).each( function(){

            var $table = $( this ),
                scrollChecker = function( el ) {
                    // Let’s see if we’ve scrolled to the top or bottom of our container, and apply status classes (topClass and bottomClass) accordingly.
                    var $el = $( el ),
                        scrollPos = $el.scrollTop();

                    if ( scrollPos == 0 ) {
                        // We’re at the TOP of our container, so let’s class it accordingly.
                        $el
                            .addClass( topClass )
                            .removeClass( bottomClass );
                    } else if ( scrollPos + $el.innerHeight() >= $el[ 0 ].scrollHeight ) {
                        // We’re at the BOTTOM of our container, so let’s class it accordingly.
                        $el
                            .addClass( bottomClass )
                            .removeClass( topClass );
                    } else {
                        // We’re somewhere in the MIDDLE of our container, so let’s remove all the position classes.
                        $el.removeClass( topClass + " " + bottomClass );
                    }
                };

            // Let’s shim our table with a bit of extra markup
            $table
                .wrap( '<div class="' + parentClass + '-inner"></div>' )
                .parent()
                .wrap( '<div class="' + parentClass + ' full"></div>' )
                .prepend( '<div class="' + parentClass + '-cap ' + parentClass + '-cap--top" role="presentation" aria-hidden></div>')
                .append( '<div class="' + parentClass + '-cap ' + parentClass + '-cap--bottom" role="presentation" aria-hidden></div>');

            // Select our new scrollable container, and attach our `scrollChecker` handler to it
            var $scrollContainer = $table.parent();

            $scrollContainer.on( "scroll", function( e ) {
                scrollChecker( e.target );
            } );

            // Initialize scrollChecker
            scrollChecker( $scrollContainer[ 0 ] );
...
(function( $ ) {
    ...
    // Currently, we’re using the ajaxInclude pattern (https://github.com/filamentgroup/Ajax-Include-Pattern) to dynamically load our Series and Topics submenus. Once those menus loaded into the page, we can trigger collapsible, and attach a few event handlers
    $( ".site-nav-item" )
    ...
    $( window ).on( "resize", function() {
        $( ".site-mast .collapsible-expanded .collapsible-header, .modal.collapsible-expanded .collapsible-header" ).trigger( "click" );
    } );

    // When the escape key is pressed, close all open modals
    $( document ).keyup( function( e ) {
        if ( e.keyCode === 27 ) {
            $( ".modal.collapsible-expanded .collapsible-header" ).trigger( "click" );
        }
    } );
...
Component: Conditionally-scrolling masthead
...
(function( $ ) {
    ...
    var componentName = "site-mast",
        enhancedAttr = "data-enhanced-" + componentName,
        initSelector = "." + componentName + ":not([" + enhancedAttr + "])";

    // Are we in a browser that supports `position: fixed`? (via fixed-fixed.js)
    var supportsFixed = $( "html" ).is( ".fixed-supported" )

    // [class] definitions
    var bodyClass = "has-fixed-masthead",
        mastClass = "is-fixed";
    ...
    $.fn[ componentName ] = function(){
        return this.each( function(){
            ...
            if ( supportsFixed ) {
                ...
                // Define a waypoint (via jquery.waypoints.min.js)
                var ceiling = new Waypoint( {
                    element: $( ".content" )[ 0 ],
                    handler: function( direction ) {
                        if ( direction === "down" ) {
                            $mast.addClass( mastClass );
                            $( "body" ).addClass( bodyClass );
                        } else {
                            $mast.removeClass( mastClass );
...
Component: Conditionally-scrolling mini-masthead
...
(function( $ ) {
    ...
    var componentName = "site-mast-mini-fixed",
        enhancedAttr = "data-enhanced-" + componentName,
        initSelector = "." + componentName + ":not([" + enhancedAttr + "])";
    ...
    // Array for visible waypoints
    var visibleWaypoints = [];

    // Which elements are we trying to avoid?
    var elements = ".table--long, .table--shortened",
        excludeElements = ".stickyclone";
        ...
        return this.each( function(){
            ...
            if ( supportsFixed ) {
                ...
                var ceiling = new Waypoint( {
                    ...
                    handler: function( direction ) {
                        if ( direction === "down" ) {
                            $mast.addClass( scrollingClass );
                        } else {
                            $mast.removeClass( scrollingClass );
                        }
                    }
                } );

                // Define a waypoint (via jquery.waypoints.min.js)
                var navCheck = function( element, action ) {
                    if ( action === "add" ) {
                        visibleWaypoints.push( element )
                        $mast.addClass( hiddenClass );
                    } else {
                        var index = visibleWaypoints.indexOf( element );

                        if ( index > -1 ) {
                            visibleWaypoints.splice( index, 1 );
                        }

                        if ( visibleWaypoints.length === 0 ) {
                            $mast.removeClass( hiddenClass );
                        }
                    }
                }

                $( elements ).not( excludeElements ).each( function() {
                    // Define a waypoint (via jquery.waypoints.min.js)
                    var inview = new Waypoint.Inview( {
                        element: $( this )[ 0 ],
                        enter: function( direction ) {
                            navCheck( this.element, "add" );
                        },
                        exited: function( direction ) {
                            navCheck( this.element, "remove" );
                        }
                    } );
                } );
...
(function( $ ) {
    ...
    var componentName = "subnav",
        enhancedAttr = "data-enhanced-" + componentName,
        initSelector = "." + componentName + ":not([" + enhancedAttr + "])";

    // Subnav should get our `position: sticky` polyfill
    stickybits( "." + componentName, {
    ...
    // Which elements are we trying to avoid?
    var elements = ".medium, .full, .site-footer, .aside.aside-medium, .aside.aside-large",
        excludeElements = ".aside--alt, .stickyclone";
        ...
        return this.each( function(){
            ...
            var navCheck = function( element, action ) {
                if ( action === "add" ) {
                    ...
                    if ( visibleWaypoints.length === 0 ) {
                        $subnav.removeClass( hiddenClass );
                    }
                }
            }
...
/*
Drop slider ranges into our Feltron Numbers™
*/
(function( $ ) {

    "use strict";

    var componentName = "number",
        enhancedAttr = "data-enhanced-" + componentName,
        initSelector = "." + componentName + ":not([" + enhancedAttr + "])";

    // Markup we’ll be inserting for each range
    var tmpl = [
    ...
    $.fn[ componentName ] = function() {
        return this.each( function(){
            var $number = $( this ),
                $rank = $( ".rank", $number ),
                $scale = $( ".rank-scale", $number );

            // We want to ensure a.) there’s not already a `.rank-scale` scale element in place, and b.) there *is* a `.rank` element
            if ( $scale.length === 0 && $rank.length > 0 ) {
                // Get the rank/total attributes from the .rank link
                var thisRank = $rank.attr( "data-rank" ),
                    thisTotal = $rank.attr( "data-total" ),
                    thisPercent = ( thisRank / thisTotal ) * 100;

                // Generate the markup to display this number’s scale
                var scale = tmpl.split( "{{ position }}" ).join( thisPercent + "%" );

                // Drop the markup into our link
                $rank.prepend( scale );
            }
        });
    };
...
// PP is our project-specific object
window.PP = {};
window.PP.utils = {};
(function( win, undefined ){
...
$( document ).ready(function() {
    ...
    // FPO - normally, this would be done much earlier in the stack, preferably as part of a “cut the mustard”-style feature check
    $("html").addClass( "enhanced" );

    // Initialize Ajax-Include pattern
    $("[data-append], [data-replace], [data-after], [data-before]").ajaxInclude();

    // Initial appendAround
...
(function( win, undefined ){
    ...
    PP.utils.featureTest = featureTest;
...
// SETUP
function makeColumnChart(positiveData, negativeData, h, elId) {
  const mergeByDate = (a1, a2) =>
    a1.map((itm) => ({
      ...a2.find((item) => item.signed_date === itm.signed_date && item),
      ...itm,
    }));

  var data = mergeByDate(positiveData, negativeData);

  var annotationsData = [
    {
      date: "February 26, 2020",
      text: ["February 26", "First reported", "death in U.S."],
      class: "db",
    },

    {
      date: "March 13, 2020",
      text: ["March 13", "State of", "emergency"],
      class: "dn db-l dn-m",
    },

    {
      date: "April 28, 2020",
      text: ["April 28", "One million", "cases"],
      class: "dn db-ns",
    },
  ];

  var margin = { top: 50, right: 20, bottom: 50, left: 44 },
    x = d3.scaleTime(),
    y = d3.scaleLinear(),
    theData = undefined,
    diffDays = undefined;

  var parentDiv = d3.select(elId).classed("relative", true);

  var svg = parentDiv.append("svg").attr("class", "w-100 " + h);
  ...
  function draw() {
    function getWidth() {
      // return Math.round(width / diffDays);
      return width / diffDays;
    }
    ...
    var backBars = barGroups.append("rect").attr("class", "back-bar o-0");

    backBars = g.selectAll(".back-bar");
    backBars
      .attr("x", function (d) {
        return xStart(d.signed_date);
      })
      .attr("y", 0)
      .attr("width", getWidth())
    ...
    var frontBars = barGroups
      .append("rect")
      .attr("class", "front-bar no-pointer-events");

    frontBars = g.selectAll(".front-bar");
    frontBars
      .attr("x", function (d) {
        return xStart(d.signed_date);
      })
      .attr("y", function (d) {
        return y(d.positive_total);
      })
      .attr("width", function (d) {
        // console.log(Math.max(getWidth() - 2, 2));
        return Math.max(getWidth() - 1, 2);
      })
      // .attr("width", getWidth())
      .attr("height", function (d) {
        return y(0) - y(d.positive_total);
      })
      .style("fill", "#34BC9A");

    var frontBarsNegative = barGroups
      .append("rect")
      .attr("class", "front-bar-negative no-pointer-events");

    frontBarsNegative = g.selectAll(".front-bar-negative");
    frontBarsNegative
      .attr("x", function (d) {
        return xStart(d.signed_date);
      })
      ...
      .attr("height", function (d) {
        return y(d.negative_total) - y(0);
      })
      .style("fill", "#AC34BC");

    var annotationsGroup = g
      .selectAll(".annotations-group")
      .data(annotationsData)
      .enter()
      .append("g")
      .attr("class", function (d) {
        return "annotations-group dn db-ns " + d.class;
      });
    annotationsGroup = g.selectAll(".annotations-group");

    annotationsGroup.attr("transform", function (d) {
      return "translate(" + xStart(new Date(d.date)) + "," + -35 + ")";
    });

    var annotationLine = annotationsGroup
      .append("rect")
      .attr("class", "annotation-line");

    annotationLine = annotationsGroup.selectAll(".annotation-line");
    ...
    var annotationText = annotationsGroup
      ...
      .attr("class", function (d, i) {
        return i == 0 ? "annotation-text f7 b" : "annotation-text f7";
      })
      .html(function (d) {
        return d;
      });

    annotationText = annotationsGroup.selectAll(".annotation-text");

    annotationText.attr("x", 10).attr("y", function (d, i) {
      return i * 16 + 10;
    });
    ...
    g.select(".axis--x")
      ...
      // .call(d3.axisBottom(x).tickSize(4, 0).tickFormat(d3.timeFormat("%b %d")))
      .call(
        d3
          .axisBottom(x)
          ...
          .ticks(
            ...
            //   console.log(d);
            //   console.log(d3.timeYear(d));
            //   return d3.timeDay.count(0, d) % 10 === 0;
            // })
            d3.timeMonth.filter((d) => {
              // console.log(d);
              // console.log(d3.timeMonth.count(0, d));
              return d3.timeMonth.count(0, d) % 10 === 1 ? false : true;
            })
          )
    ...
    g.select(".axis--y")
      .call(d3.axisLeft(y).ticks(3, "$.0s").tickSize(-width))
      .call((g) => g.select(".domain").remove())
      .call((g) =>
        g
          .selectAll(".tick line")
      ...
      .call((g) => g.selectAll(".tick text").attr("x", -4).attr("dy", 3));

    // EXIT
    annotationsGroup.exit().remove();
    barGroups.exit().remove();

    function xStart(date) {
      // return Math.round(x(new Date(date)) - getWidth() * 0.5);
      return x(new Date(date)) - getWidth() * 0.5;
    }
    function mouseout(d) {
      tooltipDiv.classed("o-0", true);
    }
    function tooltip(d) {
      tooltipDiv
        // .style("left", Math.round(x(d.signed_date)) + margin.left - 105 + "px")
        .style("left", x(d.signed_date) + margin.left - 105 + "px")
        .style("top", 0 + "px")
        .classed("o-0", false)
        .html(function () {
          return (
            "<div class='ttip'>" +
            "<div class='fw7 tnum'>" +
            formatTime(d.signed_date) +
            "</div>" +
            "<div class='f7 color-dark w-100 pt2'>" +
            "<div class='w-100 flex pb1 color-accent-dark'>" +
            "<div class='f7 mr3 w-80 fw7'>Spending</div>" +
            "<div class='tr'>" +
            formatMoney(d.positive_total) +
            "</div>" +
            "</div>" +
            "<div class='w-100 flex bt b-color-dark pt1 accent-2'>" +
            "<div class='f7 mr3 w-80 fw7'>Adjustments</div>" +
            "<div class='tr'>" +
            formatMoney(d.negative_total) +
            "</div>" +
            "</div>" +
            "</div>"
          );
    ...
    var tooltipDiv = d3
      ...
      .attr(
        "class",
        "tooltip pa2 f8 bg-light-light absolute top-0 left-0 no-pointer-events o-0 br0"
      );
  }
  ...
  function loadData(data) {
    data.forEach(function (d) {
      d.signed_date = parseDate(d.signed_date);
      d.positive_total = +d.positive_total || 0;
      d.negative_total = +d.negative_total || 0;
    });

    theData = data;
    var datesArray = theData.map(function (d) {
        return new Date(d.signed_date);
      }),
      orderedDates = datesArray.sort(function (a, b) {
        return Date.parse(a) > Date.parse(b);
      });

    var startDate = orderedDates[0];
    startDate.setDate(startDate.getDate() - 1);

    var endDate = orderedDates[orderedDates.length - 1];
    endDate.setDate(endDate.getDate() + 1);

    var staticStartDate = new Date(2020, 1, 24);
    var staticEndDate = new Date();
    var domain = [staticStartDate, staticEndDate];

    var diffTime = Math.abs(domain[0] - domain[1]);
    diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

    x.domain(domain);
    y.domain([
      d3.min(theData, function (d) {
        return d.negative_total;
      }),
      d3.max(theData, function (d) {
        return d.positive_total;
      }),
    ]);
...
// Group: collapsible-related files





// END: collapsible-related files
...
// Group: tablesaw-related files


// END: tablesaw-related files

// table.stickyheaders and table.shortened should follow tablesaw


// END table.stickyheaders and table.shortened should follow tablesaw
...
// This will include all the other javascript files you create in your app


//TABLE SAW DATE SORTER
$(function () {
  $(".date-sort").data("tablesaw-sort", function (ascending) {
    return function (a, b) {
      ...
      var dateA = new Date(a.cell);
      var dateB = new Date(b.cell);
