...
// DRUPAL THEME SCRIPTS
(function($) {
  Drupal.behaviors.d8template_core = {
    attach: function(context, settings) {
      ...
      // ALL JS Browsers ----------------------------------------------- 		
		...
		//Accessible skip links tweak
		//IN Drupal 8 this has been removed form scripts.js so it can be editable in a sub theme
		//content skip
		$('#content-toggle a').click(function(e) {
		e.preventDefault();
		$('main h1:first-of-type').attr('tabindex', '-1').focus();
		});
		
		$('main h1:first-of-type').on('blur focusout', function () {
		...
		e.preventDefault();
		$('#navigation ul').first().children(":first").children(":first").attr('tabindex', '-1').focus();
		});
		
		$('#navigation ul').first().children(":first").children(":first").on('blur focusout', function () {
			var attr = $(this).attr('tabindex');
			if (typeof attr !== typeof undefined && attr !== false) {
        	$(this).removeAttr('tabindex');
			}
...
*/
  ...
  var id = 'collapsible-' + $this.index();
	...
	// add the word toggle
	$this.append( ' <span>Toggle open</span> <svg xmlns="http://www.w3.org/2000/svg" width="12" height="6" overflow="visible"><path fill="#676767" d="M6 6L0 0h12z"/></svg>' );
	//add a marker
  // wrap the content and make it focusable
  ...
  button.on('click', function() {
	...
	*/
	...
	/*// ACCESSIBLE OFF CANVASS MENU 2.0 */
		...
		if (Modernizr.mq('(min-width: 20em)') && Modernizr.mq('(max-width: 63.9375em)')) {
	...
	jQuery('.mustardo-nav-toggle--menu.bound').click(function() {
		//get the first nav item which should be the link or a button if it panle drop down
		setTimeout(function() { jQuery('#primary-nav .start-nav-item').focus() }, 100);
	});
	jQuery('.mustardo-nav-toggle--close.bound').click(function() {
		...
		//jQuery('#primary-nav .start-nav-item').removeAttr('tabindex');
	});
	jQuery('.mustardo-nav-toggle--menu').on('blur focusout', function () {
	...
	// END MEDIA IF
		...
		//jQuery( '#menu-toggle').removeClass('mustardo-menu-toggle');
		jQuery( '#menu-toggle button' ).replaceWith('<a href="#navigation" class="toggle-control">Skip to Navigation</a>');
   ...
   // ARIA Panel Menu 
	...
	// for navigation 
	if (Modernizr.mq('(min-width: 18.75em)')) {
    ...
    if ( $('.js-panel__link').children().length == 0 ) {
	jQuery('.js-panel__link').append('<i role="presentation" class="js-panel__marker"><svg aria-hidden="true" focusable="false" xmlns="http://www.w3.org/2000/svg" width="12" height="6" viewBox="0 0 12 6" overflow="visible"><path fill="#FFF" d="M5.999 6L0 0h12z"/></svg></i>');
    }
...
function showHideTakeover () {  
	...
	jQuery('.js-takeover').on('click', '.js-close-btn',function(e){
		...
		$('#donation-form').addClass('js-donation-form');
		...
		$('#fieldset-regular .o-btn a').html('next <span class="u-visually-hidden">continue to our external donate form</span>');
		...
		//remove the amount from the other amount text box if the focus is on the radio buttons
		...
		if ('checkValidity' in form) {
			...
			//$step2.children('.fieldset-wrapper').append('<div class="form-actions"><button type="button" class="form-submit form-submit--secondary form-submit--previous">Previous <span class="u-visually-hidden">step</span></button> <a id="step-2-form-submit" href="#fieldset-step-3" class="form-submit form-submit--primary">Next <span class="u-visually-hidden">step</span></a></div>');
			$step2.children('.fieldset-wrapper').once('DonateFormBehaviour').append('<div class="form-actions"><button type="button" class="form-submit form-submit--secondary form-submit--previous">Previous <span class="u-visually-hidden">step</span></button> <button type="button" id="step-2-form-submit" class="form-submit form-submit--primary form-submit--next">Next <span class="u-visually-hidden">step</span></button></div>');
			
			
			//Add Previous to the end of the form we are going to add this to the very end of the form
			$step3.once('DonateFormBehaviour').children('.fieldset-wrapper').append('<div id="form-actions-final" class="form-actions u-hidden" aria-hidden="true"><button type="button" class="form-submit form-submit--secondary form-submit--previous">Previous <span class="u-visually-hidden">step</span></button></div>');
			...
			function createStepCounter(){
				...
				for(var i = 0; i < $steps.length; i++){
						if(i === activeStep){
							counters += '<i class="js-step-counter__item is-active" />';
						}
						else if (i < activeStep){
							counters += '<i class="js-step-counter__item is-complete" />';
						}
						else{
							counters += '<i class="js-step-counter__item" />';
						}
					}
			...
			//STEP 1
			//So there is always something selected in fieldset one so there is no need to validate it
				...
				var invalids = false;
					 ...
					 invalids = fields.filter(function(){
						...
						if(invalids.length){
							...
							//Focus on the legend of step 3
							$('#fieldset-step-3 .fieldset--payment legend').attr({'aria-selected' : 'true','tabindex' : '0'});
							$('#fieldset-step-3 .fieldset--payment legend').focus();
			...
			$('#form-actions-final .form-submit--previous').click(function(){
				...
				$(this).parent().addClass('u-hidden').attr('aria-hidden', true);
				//reset to defaults
				...
				//remove the focusing on 
				$('#fieldset-step-3 .fieldset--payment legend').removeAttr('aria-selected').removeAttr('tabindex');
...
$('[role="tab"]').on('click', function(e) {
  ...
  $('#' + $(this).attr('href').substring(1))
    .attr('aria-hidden', null);
		...
		//ACCESSIBLE AJAX FILTER ================================= */
		//Accessible ajax filter content add the alerts
		if (!$('.d-section--style-feed div[aria-live="assertive"]').length) {
			jQuery('.d-section--style-feed .d-feed__filters').parent().before('<div aria-live="assertive" role="alert" class=""></div>');
		}
		
		var liveElement = jQuery('.d-section--style-feed div[aria-live="assertive"]');
		
		jQuery('.d-section--style-feed .d-feed__filters form .filters__submit input:not(.bound)').addClass('bound').click(function(){
			//alert('Please wait. Loading products.')
			liveElement.text('Please wait. Filtering content.');
			liveElement.removeClass('fade-out u-visually-hidden');
			liveElement.addClass('fade-in messages status');
		});
		//On the ajax load 
		if (Drupal.ajax) {
    		jQuery('.d-section--style-feed .d-feed__filters form').ajaxComplete(function(event, xhr, settings) {
			//JS stuff
			var returnedItems = jQuery('.d-section--style-feed .d-feed__filters + .d-feed__body article').length;
			liveElement.text('Filtering complete. ' + returnedItems +' items found.');
			if (returnedItems == 0) {
				liveElement.removeClass('status').addClass('warning')
			}
			setTimeout(function(){ liveElement.addClass('fade-out'); }, 800);
			setTimeout(function(){ liveElement.addClass('u-visually-hidden'); }, 1600);
    		});
  		}
		
		//END ACCESSIBLE AJAX FILTER ================================= */
        ...
        function loadExternalVideo() {
            ...
            $('.js-play-btn').click(function(event){
                ...
                if (this.href.indexOf("youtube") > -1) {
                    ...
                    $(this).closest('.js-splash--video').html('<div class="js-splash__video-embed video-embed video-embed--16-9"><div class="video-embed__wrapper"><iframe id="' + splashIframeId + '-'+ iframeVideoID +'" src="' + iframeVideoURl + '?autoplay=1&modestbranding=1" frameborder="0" scrolling="no" allow="autoplay https://www.youtube-nocookie.com"></iframe></div></div>');
                    
                }

                else {
                    //this will be vimeo
                    $(this).closest('.js-splash--video').html('<div class="js-splash__video-embed video-embed video-embed--16-9"><div class="video-embed__wrapper"><iframe id="video-splash-'+ iframeVideoID +'" src="' + iframeVideoURl + '?autoplay=1" frameborder="0" scrolling="no" allow="autoplay" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></div></div>');
        ...
        //RESIZE FUNCTIONS
        /* this causes the auto close bug in IOS
		var widthHorz ='';
