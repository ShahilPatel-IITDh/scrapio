(function($) {
	...
	function coverSlider(){
		...
		$slider.each(function(){
			...
			if ( itemLength > 1 ){
				$this.prepend('<button type="button" class="prev">ì´ì </button><button type="button" class="next">ë¤ì</button>');
	...
	function viewMore(){
		if ( $(".paging-view-more").length && $(".post-item").length ){
			...
			function viewMoreShow(){
				...
				$(".pagination a").hide();
				if( nextUrl ){
					$(".pagination").append('<a href="'+nextUrl+'" class="btn view-more">ëª©ë¡ ëë³´ê¸°</a>');
