(function () {
  ...
  */
  ...
  var systemIconsNewNames = {
    ...
    'pencil-ruler-large': 'skills-medium',
    'pencil-ruler-small': 'skills-medium',
  ...
  var largeIconsInMercado = ['shield-large', 'visibility-large', 'visibility-off-large', 'star-fill-large', 'star-outline-large', 'star-half-large', 'play-large'];
  function convertToMercado(newType, category) {
    ...
    if (category === 'ui' || category === 'nav') {
      ...
      if (largeIconsInMercado.indexOf(newType) > -1) {
        // mercado comes in 3 sizes (small - 16, medium - 24, large - 32) while AD
        // only in two sizes (small - 16, large - 24), so for those icons that
        // contain large variants in mercado, we shouldn't change the size
        return newType;
      }
      // in all other cases, map large to medium
  ...
  var MERCADO_LARGE_ICON_ROOT_NAMES = largeIconsInMercado.map(function (i) {
    return i.replace('-large', '');
  });

  function handleSize(size, iconName) {
    if (size === undefined) size = 'large';

    // Default to assuming that only small and large icons are valid,
    // in line with Art Deco behavior.
    // DON'T PANIC. If we're using a Mercado sprite, we'll convert this
    // to medium later on, in the convertToMercado() method.
    var availableSizes = { small: 1, large: 1 };

    // Buuut, if this is in fact an icon that has a large version in Mercado,
    // we should preserve medium if it's defined so we don't erroneously
    // upsize it to large.
    // (We'll convert this to large in findIconInSVG() if we find out that
    // we're not using a Mercado icon. Sadly, we don't know until we try to load
    // the icon from the SVG if this'll be an Art Deco or Mercado icon.
    if (MERCADO_LARGE_ICON_ROOT_NAMES.includes(iconName)) {
      availableSizes.medium = 1;
    }
  ...
  function getIconCategory(type) {
    ...
    if (scalingIcons[type] || type.includes('premium-wordmark') || type.includes('premium-badge') || type.includes('linkedin-bug') && type.includes('on-dark')) {
      return 'scaling';
    }
    if (socialIcons[type]) {
      return 'social';
  ...
  var api = {
    ...
    findIconInSVG: function findIconInSVG(allIcons, newType, type, category, options) {
      ...
      if (!iconSource) {
        // If we're here, this is going to be an Art Deco icon.
        // If we still happen to have a medium UI icon here, we should uplevel
        // it to large, since Art Deco doesn't have medium UI icons.
        var upleveledType = category === 'ui' || category === 'system-icons' ? newType.replace('-medium', '-large') : newType;
        ...
        if (defs && defs.querySelector) {
          iconSource = defs.querySelector('[id="'.concat(upleveledType, '"]'));
        } else if (defs) {
          // Look for a <svg> el with the ID and return if found
          iconSource = this.findElementInNodeListById([].slice.call(defs.getElementsByTagName('svg')), upleveledType);
        }
        iconSource = iconSource && withoutIds(iconSource);
  ...
  !(function (e, t) {
  function n() {
      var e = A.splice(0, A.length);for (et = 0; e.length;) e.shift().call(null, e.shift());
    }function r(e, t) {
      ...
      for (var t = this; t && t.nodeName !== e;) t = t.parentNode;return t;
    },
        je = Re.dispatchEvent,
        Ge = Re.getAttribute,
        ze = Re.hasAttribute,
    ...
    }).observe(P, { childList: !0, subtree: !0 }), P.innerHTML = ""), tt || (Me || Ee ? (S = function (e, t) {
      fe.call(t, e) || d(e, t);
    }, V = d) : (S = function (e, t) {
      e[_] || (e[_] = C(!0), d(e, t));
