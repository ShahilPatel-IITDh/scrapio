...
if(main.decSep==','){main.decType='c'}else{main.decType=''}
...
return stack.join('/')}
function decfmt(){if(main.decSep==','){fixSpells(document.body,'d')
...
let str='<div style="position: absolute; right: 50%; margin-right: 260px; top:120px; font:45px Arial; background-color: rgba(255, 220, 0, 0.5);  border-radius:5px; border: 1px solid red; "><a href="'+
path+
...
until.setTime(until.getTime()+80*60*60*1000)
let now=new Date()
...
adsSet(true)}
function adsDo(){let adType='adsense'
if(adType=='adsense'){adsenseDo()}
if(adType=='dummy'){adsDummy()}}
function adsDummy(){let dummy=document.createElement('img')
...
function adsenseDo(){let script=document.createElement('script')
...
function doQLocal(id,s){let wd=Math.min(window.innerWidth-50,800)
let urlStt=urlSttGet()
...
adsHideSet()
if(adOffQ()){}else{setTimeout(adsDo,2000)}
doLocal()
...
for(let i=0;i<qs.length;i++){s+='<a href="javascript:doQ('+qs[i]+",'"+joinedStr+"'"+')">Q'+(i+1)+'</a> '}
let path=location.pathname
path=path.split('/mathsisfun/').join('/')
let dir=path.substring(0,path.lastIndexOf('/'))
console.log('dir',path,dir)
if(path=='/mean.html'){s+='<br>'
s+='<span style="display:inline-block; min-width:8rem;">Local popup:</span>'
for(let i=0;i<qs.length;i++){s+='<a href="javascript:doQLocal('+qs[i]+",'"+joinedStr+"'"+')">Q'+(i+1)+'</a> '}}
return s}
...
if(str.trim().length<=1)return
iframeDo(node,str)}
function putFlash6(w,h,fn,querystring,clr){if(!querystring){querystring="";}else{querystring="?"+querystring;}
if(!clr){clr="#d6d9e6";}
...
if(div!=null){let btnStr='<button type="button" class="btn" style="z-index:2; position:absolute; right:0; bottom:0;" onclick="iframeMax(\''+filename+"',"+wd+','+ht+')">&rect;</button>'
