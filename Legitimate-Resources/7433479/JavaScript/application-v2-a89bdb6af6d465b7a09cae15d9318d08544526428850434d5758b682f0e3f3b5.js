...
document.addEventListener('DOMContentLoaded', () => {
  const characterCounts = document.querySelectorAll('.js-character-count');

  function handleInput(e) {
    const valueLength = e.target.value.length;
    const { descriptionElement, displayElement, maxLength, warning } = this;
    displayElement.innerText = `${valueLength}/${maxLength}${
      valueLength > maxLength && warning ? ` ${warning}` : ''
    }`;
    if (valueLength <= maxLength) {
      descriptionElement.innerText = I18n.t(
        'js.character_count.characters_remaining',
        { number: maxLength - valueLength },
      );
    }
    if (valueLength > maxLength) {
      descriptionElement.innerText = `${I18n.t(
        'js.character_count.characters_exceeding',
        { number: valueLength - maxLength },
      )}.${warning ? ` ${warning}` : ''}`;
    }
  }

  function getHandleInput(elems) {
    return handleInput.bind(elems);
  }

  if (characterCounts.length > 0) {
    characterCounts.forEach(input => {
      const descriptionElementId = input.getAttribute('aria-describedby');
      const descriptionElement = document.getElementById(descriptionElementId);
      const displayElement = document.querySelector(
        `[data-count-display-for="${descriptionElementId}"]`,
      );
      const maxLength =
        input.getAttribute('maxlength') || input.dataset.maxlength;
      const warning = input.dataset.warning;

      if (!descriptionElementId) {
        console.log(
          `Could not initiate character count input. No 'aria-describedby' specified`,
        );
        return;
      }
      ...
      if (!descriptionElement) {
        console.log(
          `Could not initiate character count input. No description element matching 'aria-describedby'`,
        );
        return;
      }
      ...
      if (!displayElement) {
        console.log(
          `Could not initiate character count input. No display element matching '[data-count-display-for="${descriptionElementId}"]'`,
        );
        return;
      }
      ...
      if (!maxLength) {
        console.log(
          `Could not initiate character count input. No 'maxlength' attribute specified`,
        );
        return;
      }
      ...
      input.addEventListener('input', getHandleInput(context));
      handleInput.call(context, { target: input });
    });
  }
});
document.addEventListener('DOMContentLoaded', () => {
...
document.addEventListener('DOMContentLoaded', () => {
  $('.js-validate').validate({
    ...
    unhighlight: element => {
      ...
      $(element).next('.field_helper-text').show();
    },
    errorPlacement: (error, element) => {
      if (element.next('.field_helper-text')) {
        element.next('.field_helper-text').hide();
...
$(document).ready(function () {
  function showMessage(service, message = '', success = false) {
    ...
    if (success) {
      $(`#${service}-integration-error`).removeClass('field_error');
      $(`#${service}-integration-error`).addClass('field_success');
    } else {
      $(`#${service}-integration-error`).addClass('field_error');
      $(`#${service}-integration-error`).removeClass('field_success');
    }
    $(`#${service}-integration-error`).html(message);
  }

  $('form.create-integration')
    .on('ajax:beforeSend', function (event) {
      showMessage(event.currentTarget.dataset['service'], '');
    })
    .on('ajax:success', function (event) {
      ...
      const onmessage = e => {
        if (e.data.type === 'tray.configPopup.error') {
          configFinished = true;
          configWindow.close();
          showMessage(service, `Error: ${e.data.err}.`);
        }
        if (e.data.type === 'tray.configPopup.cancel') {
          configFinished = true;
          configWindow.close();
          showMessage(service, 'Integration cancelled, please try again.');
        }
        if (e.data.type === 'tray.configPopup.finish') {
          configFinished = true;
          configWindow.close();
          $(`#verify-${service}-integration`).submit();
          showMessage(service, 'Integration finished, please wait a few seconds...', true);
        }
        if (e.data.type === 'tray.configPopup.validate') {
          // Not added in the url from the backend for now
        }
      };
      ...
      const checkWindow = () => {
        if (configWindow && !configWindow.closed)
          return setTimeout(checkWindow, 1000); // Recheck if window has been closed

        if (!configFinished)
          showMessage(service,'Integration not finished, please try again');
    ...
    .on('ajax:error', function (event) {
      ...
      showMessage(
        service,
        'We are unable to proceed with this integration, please try again later',
      );
    });
});
...
function previewCoverImage(input, targetId) {
  jQuery('img.cover-image').removeClass('-active');
  previewImage(input, targetId);
  jQuery('#cover-image-button').text('Change image');
}

function chooseCoverImage(image) {
  jQuery('img.cover-image').removeClass('-active');
  jQuery('#cover-image-button').text('Choose image');

  newCoverImage = jQuery(image);
  newCoverImage.addClass('-active');

  jQuery('#organization_default_cover_image').val(newCoverImage.attr('id'));
  jQuery('#organization_cover_image').val('');
  jQuery('#organization-cover-image').removeAttr('src');
};
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('[data-require-confirmation]').forEach(button => {
    button.addEventListener('click', event => {
      if (!document.getElementById(button.dataset.requireConfirmation).checked) {
        document.getElementById(button.dataset.confirmationError).style.display = 'inline'
        event.preventDefault()
        event.stopPropagation()
      } else {
        document.getElementById(button.dataset.confirmationError).style.display = 'none'
      }
    })
  })
});
function scrollSpy(selector, activeClass) {
  ...
  const elements = [...links].map(link => document.querySelector(link.hash))

  window.onscroll = function() {
    const scrollPosition = document.documentElement.scrollTop || document.body.scrollTop

    elements.forEach(element => {
      if (element.offsetTop <= scrollPosition + 60) {
        document.querySelector(`${selector}.${activeClass}`).classList.toggle(activeClass)
        document.querySelector('a[href*=' + element.id + ']').classList.toggle(activeClass)
      }
    })
  }
};
...
// pod-identity are the owners of this file.
// it should be removed once we fully migrate to HopinID.

function track_segment_event(track_event, auth_method) {
  if (track_event == 'Sign-in started')
    $.ajax({
      type: 'POST',
      url: '/track_sign_in_started',
      data: {
...
//
// TO DO: Move these to Webpacker and use NPM packages
