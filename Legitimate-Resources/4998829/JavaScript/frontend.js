...
a4jDefine('fpr_app', ['jquery', 'ko'], function($, ko){
	'use strict';
    ...
    var app = {
        ...
        showMessage: function(msg, type='info'){
            msg = '<p>'+msg+'</p>';
            $(':focus').blur();
            var existing = $('.fpr-message-win');
            if(existing.length){
                msg = existing.find('.fpr-win-body').html()+msg;
                existing.remove();
            }
			var html = '<div class="fpr-shade fpr-message-win"><div class="fpr-win  fpr-type-'+type+'">';
			html+= '<div class="fpr-win-body">'+msg+'</div>';
			html+= '<div class="fpr-win-close">+</div>';
			html+= '</div></div>';
			var win = $(html).appendTo('body');
			var modal = win.find('.fpr-win');
			...
			win.on('click',function(e){
				var target = $(e.target);
				if(target.is('.fpr-shade, .fpr-win-close') || target.parent().is('.fpr-win-close')){
					win.trigger('close');
				}
			});
			win.css({
        ...
        getMessageHTML: function(msg, type='info'){
            return '<div class="fpr-message fpr-type-'+type+'">'+msg+'</div>';
        },
...
(function($) {
    ...
    window.FacebookPageReviews = window.FacebookPageReviews||{};

    var Slick = window.FacebookPageReviews.slick = (function() {
        ...
        function Slick(element, settings) {
            ...
            _.defaults = {
                ...
                arrows: true,
                asNavFor: null,
                prevArrow: '<button class="fpr-slick-prev" aria-label="Previous" type="button">Previous</button>',
                nextArrow: '<button class="fpr-slick-next" aria-label="Next" type="button">Next</button>',
                autoplay: false,
                autoplaySpeed: 3000,
                centerMode: false,
                centerPadding: '50px',
    ...
    Slick.prototype.activateADA = function() {
        ...
        _.$slideTrack.find('.fpr-slick-active').attr({
            'aria-hidden': 'false'
    ...
    Slick.prototype.buildArrows = function() {
        ...
        if (_.options.arrows === true ) {
            ...
            _.$prevArrow = $(_.options.prevArrow).addClass('fpr-slick-arrow');
            _.$nextArrow = $(_.options.nextArrow).addClass('fpr-slick-arrow');

            if( _.slideCount > _.options.slidesToShow ) {

                _.$prevArrow.removeClass('fpr-slick-hidden').removeAttr('aria-hidden tabindex');
                _.$nextArrow.removeClass('fpr-slick-hidden').removeAttr('aria-hidden tabindex');
    ...
    Slick.prototype.buildDots = function() {
        ...
        if (_.options.dots === true && _.slideCount > _.options.slidesToShow) {
            ...
            _.$slider.addClass('fpr-slick-dotted');
            ...
            _.$dots.find('li').first().addClass('fpr-slick-active');
    ...
    Slick.prototype.buildOut = function() {
        ...
        _.$slides =
            _.$slider
                .children( _.options.slide + ':not(.fpr-slick-cloned)')
                .addClass('fpr-slick-slide');
        ...
        _.$slider.addClass('fpr-slick-slider');

        _.$slideTrack = (_.slideCount === 0) ?
            $('<div class="fpr-slick-track"/>').appendTo(_.$slider) :
            _.$slides.wrapAll('<div class="fpr-slick-track"/>').parent();

        _.$list = _.$slideTrack.wrap(
            '<div class="fpr-slick-list"/>').parent();
        ...
        $('img[data-lazy]', _.$slider).not('[src]').addClass('fpr-slick-loading');
    ...
    Slick.prototype.cleanUpEvents = function() {
        ...
        $(window).off('orientationchange.slick.fpr-slick-' + _.instanceUid, _.orientationChange);

        $(window).off('resize.slick.fpr-slick-' + _.instanceUid, _.resize);

        $('[draggable!=true]', _.$slideTrack).off('dragstart', _.preventDefault);

        $(window).off('load.slick.fpr-slick-' + _.instanceUid, _.setPosition);
    ...
    Slick.prototype.destroy = function(refresh) {
        ...
        $('.fpr-slick-cloned', _.$slider).detach();
        ...
        if ( _.$prevArrow && _.$prevArrow.length ) {
            ...
            _.$prevArrow
                .removeClass('fpr-slick-disabled fpr-slick-arrow fpr-slick-hidden')
        ...
        if (_.$slides) {
            ...
            _.$slides
                .removeClass('fpr-slick-slide fpr-slick-active fpr-slick-center fpr-slick-visible fpr-slick-current')
        ...
        _.$slider.removeClass('fpr-slick-slider');
        _.$slider.removeClass('fpr-slick-initialized');
        _.$slider.removeClass('fpr-slick-dotted');
    ...
    Slick.prototype.getLeft = function(slideIndex) {
        ...
        if (_.options.variableWidth === true) {
            ...
            if (_.slideCount <= _.options.slidesToShow || _.options.infinite === false) {
                targetSlide = _.$slideTrack.children('.fpr-slick-slide').eq(slideIndex);
            } else {
                targetSlide = _.$slideTrack.children('.fpr-slick-slide').eq(slideIndex + _.options.slidesToShow);
            }
            ...
            if (_.options.centerMode === true) {
                if (_.slideCount <= _.options.slidesToShow || _.options.infinite === false) {
                    ...
                    targetSlide = _.$slideTrack.children('.fpr-slick-slide').eq(slideIndex + _.options.slidesToShow + 1);
                }
    ...
    Slick.prototype.getSlideCount = function() {
        ...
        if (_.options.swipeToSlide === true) {
            ...
            _.$slideTrack.find('.fpr-slick-slide').each(function(index, slide) {
    ...
    Slick.prototype.init = function(creation) {
        ...
        if (!$(_.$slider).hasClass('fpr-slick-initialized')) {

            $(_.$slider).addClass('fpr-slick-initialized');
    ...
    Slick.prototype.initADA = function() {
        ...
        _.$slides.add(_.$slideTrack.find('.fpr-slick-cloned')).attr({
            'aria-hidden': 'true',
            'tabindex': '-1'
        }).find('a, input, button, select').attr({
            'tabindex': '-1'
        ...
        if (_.$dots !== null) {
            _.$slides.not(_.$slideTrack.find('.fpr-slick-cloned')).each(function(i) {
                var slideControlIndex = tabControlIndexes.indexOf(i);

                $(this).attr({
                    'role': 'tabpanel',
                    'id': 'fpr-slick-slide' + _.instanceUid + i,
                    'tabindex': -1
                });

                if (slideControlIndex !== -1) {
                   var ariaButtonControl = 'fpr-slick-slide-control' + _.instanceUid + slideControlIndex
            ...
            _.$dots.attr('role', 'tablist').find('li').each(function(i) {
                ...
                $(this).find('button').first().attr({
                    'role': 'tab',
                    'id': 'fpr-slick-slide-control' + _.instanceUid + i,
                    'aria-controls': 'fpr-slick-slide' + _.instanceUid + mappedSlideIndex,
    ...
    Slick.prototype.initializeEvents = function() {
        ...
        $(window).on('orientationchange.slick.fpr-slick-' + _.instanceUid, $.proxy(_.orientationChange, _));

        $(window).on('resize.slick.fpr-slick-' + _.instanceUid, $.proxy(_.resize, _));

        $('[draggable!=true]', _.$slideTrack).on('dragstart', _.preventDefault);

        $(window).on('load.slick.fpr-slick-' + _.instanceUid, _.setPosition);
        $(_.setPosition);
    ...
    Slick.prototype.lazyLoad = function() {
            ...
            $('img[data-lazy]', imagesScope).each(function() {
                ...
                imageToLoad.onerror = function() {
                    ...
                    image
                        ...
                        .removeClass( 'fpr-slick-loading' )
                        .addClass( 'fpr-slick-lazyload-error' );
        ...
        loadRange = _.$slider.find('.fpr-slick-slide').slice(rangeStart, rangeEnd);

        if (_.options.lazyLoad === 'anticipated') {
            var prevSlide = rangeStart - 1,
                nextSlide = rangeEnd,
                $slides = _.$slider.find('.fpr-slick-slide');
        ...
        if (_.slideCount <= _.options.slidesToShow) {
            cloneRange = _.$slider.find('.fpr-slick-slide');
            loadImages(cloneRange);
        } else
        if (_.currentSlide >= _.slideCount - _.options.slidesToShow) {
            cloneRange = _.$slider.find('.fpr-slick-cloned').slice(0, _.options.slidesToShow);
            loadImages(cloneRange);
        } else if (_.currentSlide === 0) {
            cloneRange = _.$slider.find('.fpr-slick-cloned').slice(_.options.slidesToShow * -1);
            loadImages(cloneRange);
        }
    ...
    Slick.prototype.loadSlider = function() {
        ...
        _.$slider.removeClass('fpr-slick-loading');
    ...
    Slick.prototype.setDimensions = function() {
        ...
        if (_.options.vertical === false && _.options.variableWidth === false) {
            ...
            _.$slideTrack.width(Math.ceil((_.slideWidth * _.$slideTrack.children('.fpr-slick-slide').length)));

        } else if (_.options.variableWidth === true) {
            _.$slideTrack.width(5000 * _.slideCount);
        } else {
            _.slideWidth = Math.ceil(_.listWidth);
            _.$slideTrack.height(Math.ceil((_.$slides.first().outerHeight(true) * _.$slideTrack.children('.fpr-slick-slide').length)));
        }

        var offset = _.$slides.first().outerWidth(true) - _.$slides.first().width();
        if (_.options.variableWidth === false) _.$slideTrack.children('.fpr-slick-slide').width(_.slideWidth - offset);
    ...
    Slick.prototype.setProps = function() {
        ...
        if (_.positionProp === 'top') {
            _.$slider.addClass('fpr-slick-vertical');
        } else {
            _.$slider.removeClass('fpr-slick-vertical');
        }
    ...
    Slick.prototype.setSlideClasses = function(index) {
        ...
        allSlides = _.$slider
            .find('.fpr-slick-slide')
            .removeClass('fpr-slick-active fpr-slick-center fpr-slick-current')
            .attr('aria-hidden', 'true');

        _.$slides
            .eq(index)
            .addClass('fpr-slick-current');
            ...
            if (_.slideCount > _.options.slidesToShow) {
                ...
                for (i = _.slideCount; i > (_.slideCount -
                    ...
                    $(_.$slides[slideIndex]).clone(true).attr('id', '')
                        ...
                        .prependTo(_.$slideTrack).addClass('fpr-slick-cloned');
                }
                for (i = 0; i < infiniteCount  + _.slideCount; i += 1) {
                    slideIndex = i;
                    $(_.$slides[slideIndex]).clone(true).attr('id', '')
                        .attr('data-slick-index', slideIndex + _.slideCount)
                        .appendTo(_.$slideTrack).addClass('fpr-slick-cloned');
                }
                _.$slideTrack.find('.fpr-slick-cloned').find('[id]').each(function() {
                    $(this).attr('id', '');
                });
    ...
    Slick.prototype.selectHandler = function(event) {
        ...
        var targetElement =
            $(event.target).is('.fpr-slick-slide') ?
                $(event.target) :
                $(event.target).parents('.fpr-slick-slide');
    ...
    Slick.prototype.startLoad = function() {
        ...
        _.$slider.addClass('fpr-slick-loading');
    ...
    Slick.prototype.unload = function() {
        ...
        $('.fpr-slick-cloned', _.$slider).remove();
        ...
        _.$slides
            .removeClass('fpr-slick-slide fpr-slick-active fpr-slick-visible fpr-slick-current')
            .attr('aria-hidden', 'true')
            .css('width', '');
    ...
    Slick.prototype.updateArrows = function() {
        ...
        if ( _.options.arrows === true &&
            ...
            _.$prevArrow.removeClass('fpr-slick-disabled').attr('aria-disabled', 'false');
            _.$nextArrow.removeClass('fpr-slick-disabled').attr('aria-disabled', 'false');

            if (_.currentSlide === 0) {

                _.$prevArrow.addClass('fpr-slick-disabled').attr('aria-disabled', 'true');
                _.$nextArrow.removeClass('fpr-slick-disabled').attr('aria-disabled', 'false');

            } else if (_.currentSlide >= _.slideCount - _.options.slidesToShow && _.options.centerMode === false) {

                _.$nextArrow.addClass('fpr-slick-disabled').attr('aria-disabled', 'true');
                _.$prevArrow.removeClass('fpr-slick-disabled').attr('aria-disabled', 'false');
...
a4jDefine('fpr_widget', ['fpr_app', 'jquery', 'ko'], function(App, $, ko){
	'use strict';
    ...
    var widget = App.extend({
        ...
        onInit: function(){
            ...
            this.reviewsAll = ko.observableArray();
            ...
            this.getCSSClass = ko.computed(function(){
                return 'fpr-widget fpr-theme-'+this.getParamValue('main_display_theme', '');
            },this);

            this.getReviewsCSSClass = ko.computed(function(){
                return 'fpr-reviews-items fpr-layout-'+this.getParamValue('reviews_layout');
            },this);

            this.getReviewCSSClass = ko.computed(function(){
                return 'fpr-reviews-review fpr-style-'+this.getParamValue('review_style')+' fpr-shape-'+this.getParamValue('review_author_pic_shape');
            },this);
            ...
            this.getRatingScoreClass = ko.computed(function(){
                return 'fpr-widget-box-head-honor-rating-score '+(String(this.getRatingScoreValue()).length==1?'fpr-single':'');
            },this);
            ...
            this.getStarsHTML = ko.computed(function(){
                ...
                var html = '';
                for(var i=1;i<=5;i++){
                    var starClass = 'fpr-icon-star-o';
                    if(score>=i-0.2){
                        starClass = 'fpr-icon-star';
                    }
                    else if(score>=i-0.7){
                        starClass = 'fpr-icon-star-half-o';
                    }
            ...
            this.cssStyles = ko.computed(function(){
                ...
                var styles = ''
                    ...
                    +'#'+cssID+' .fpr-widget-box-head{background-color:'+this.getParamValue('header_bg_color')+';color:'+this.getParamValue('header_text_color')+';}'
                    +'#'+cssID+' .fpr-widget-box-request{background:'+this.getParamValue('header_request_bg_color')+';color:'+this.getParamValue('header_request_text_color')+';}'
                    +'#'+cssID+' .fpr-widget-box-request .fpr-btn-primary{background:'+this.getParamValue('header_request_btn_bg_color')+';color:'+this.getParamValue('header_request_btn_text_color')+';}'
                    +'#'+cssID+' .fpr-reviews-container{'+(cssHeight=='none'?'overflow:hidden':'max-height:'+cssHeight)+';}'
                    
                    +'#'+cssID+' .fpr-reviews-items{margin-left:-'+postsGutterHalf+'px;margin-right:-'+postsGutterHalf+'px;}'
                    +'#'+cssID+' .fpr-reviews-items .fpr-reviews-item-inner{padding-left:'+postsGutterHalf+'px;padding-right:'+postsGutterHalf+'px;padding-bottom:'+postsGutter+'px;}'
                    
                    +'#'+cssID+' .fpr-reviews-review-author-pic{border-color:'+this.getParamValue('review_accent_color')+';}'
            
                    +'#'+cssID+' .fpr-style-card .fpr-reviews-review-quote{background:'+this.getParamValue('review_accent_color')+';}'
                    +'#'+cssID+' .fpr-style-spotlight .fpr-reviews-review-text-container{background:'+this.getParamValue('review_accent_color')+';}'
                    +'#'+cssID+' .fpr-style-spotlight .fpr-reviews-review-head-dots, #'+cssID+' .fpr-style-spotlight .fpr-reviews-review-head-dots::before, #'+cssID+' .fpr-style-spotlight .fpr-reviews-review-head-dots::after{background:'+this.getParamValue('review_accent_color')+';}'
                    
                ;
                
                if(this.getParamValue('header_cover_show')){
                    styles+= '#'+cssID+' .fpr-widget-box-head{background-image:url('+this.getDataValue('cover','')+');}';
                    if(!this.getParamValue('header_honor_show') && !this.getParamValue('header_title_show') && !this.getParamValue('header_subtitle_show')){
                        styles+= '#'+cssID+' .fpr-widget-box-head{padding-top:25%;padding-bottom:0;}';
                    }
                }
                
                if(layout!='carousel'){
                    styles+= '#'+cssID+' .fpr-reviews-items .fpr-reviews-item{width:'+widthPercent+'%;}';
                }
        ...
        getCSSID: function(){
            return 'fpr-widget-'+this.getConfigValue('id', 0);
        },
        
        getReviewTypeClass: function(review){
            return 'fpr-reviews-review-type fpr-type-'+review.recommendation_type;
        },
        
        getReviewTypeHTML: function(review){
            if(review.recommendation_type=='negative'){
                return '<i class="fpr-icon-thumbs-o-down"></i>';
            }
            else{
                return '<i class="fpr-icon-thumbs-o-up"></i>';
            }
        },
        ...
        requestActionYes: function(){
            window.open('http://facebook.com/'+this.getDataValue('id','')+'/reviews');
        },
        
        requestActionNo: function(){
            if(this.getParamValue('header_request_action', 'hide')=='show_msg'){
                this.jqDomNode.find('.fpr-widget-box-request-action').hide();
                this.jqDomNode.find('.fpr-widget-box-request-message').fadeIn(300);
            }
            else{
                this.jqDomNode.find('.fpr-widget-box-request').slideUp(300);
            }
        },
        
        haveMore: function(){
            return (this.reviewsAll().length>this.reviews().length);
        },
        ...
        _checkGridItems: function(){
            if(this.jqDomNode.find('.fpr-reviews-item').length==0){
                return;
            }
            var layout = this.getParamValue('reviews_layout', 'grid');
            var feed = this;
            ...
            this.jqDomNode.find('.fpr-reviews-review').css('height','auto');
            if(layout=='masonry'){
                if(this.layoutMasonry && this.layoutMasonry.length){
                    var newItems = this.jqDomNode.find('.fpr-reviews-item.fpr-hidden');
                    if(newItems.length){
                        this.layoutMasonry.masonry('appended', newItems.removeClass('fpr-hidden'));
                    }
                    this.layoutMasonry.masonry('layout');
                }
                else{
                    this.jqDomNode.find('.fpr-reviews-item.fpr-hidden').removeClass('fpr-hidden');
                    this.layoutMasonry = this.jqDomNode.find('.fpr-reviews-items.fpr-layout-masonry').masonry({
                        'itemSelector': '.fpr-reviews-item',
                        'percentPosition': true,
                        'columnWidth': '.fpr-reviews-item',
                        'masonry':{
                            'columnWidth': '.fpr-reviews-item'
                        }
                    });
                }
                var images = this.jqDomNode.find('.fpr-reviews-item img');
                var imagesLoaded = 0;
                var imagesTotal = images.length;
                var imgLoaded = function(img){
                    imagesLoaded++;
                    $(img).removeClass('fpr-img-loading').addClass('fpr-img-loaded');
                ...
                if(!this.layoutCarousel){
                    ...
                    var slickCfg = {
                        'prevArrow': '<span data-role="none" class="fpr-slick-prev" aria-label="Previous" tabindex="0" role="button"><i class="ing-icon-angle-left ing-fw"></i></span>',
                        'nextArrow': '<span data-role="none" class="fpr-slick-next" aria-label="Next" tabindex="0" role="button"><i class="ing-icon-angle-right ing-fw"></i></span>',
                        'adaptiveHeight': true,
                        'dots': true,
                        'arrows': false,
                    ...
                    this.layoutCarousel = this.jqDomNode.find('.fpr-reviews-items.fpr-layout-carousel').fprSlick(slickCfg);
                }
            }
            else{ // layout=='grid'
                this.jqDomNode.find('.fpr-reviews-item.fpr-hidden').removeClass('fpr-hidden').hide().fadeIn(300);
                
                var rows = [];
                var row = -1;
                this.jqDomNode.find('.fpr-reviews-review').each(function(index){
                    if(index%cols==0){
                        row++;
                    }
                    var el = $(this);
        ...
        destroyLayout: function(){
            ...
            this.currentPage = 1;
            this.showReviewsPage(this.currentPage);
        },
        ...
        getCSSHeight: function(){
            ...
            if(height.substr(-2)=='px'){
                ...
                if(this.getParamValue('header_show')){
                    subsHeight+= this.jqDomNode.find('.fpr-widget-box-head').height();
                }
                if(this.getParamValue('header_request_show')){
                    subsHeight+= this.jqDomNode.find('.fpr-widget-box-request').height();
                }
        ...
        showReviewsPage: function(_page){
            var layout = this.getParamValue('reviews_layout', 'grid');
            this.currentPage = _page;
            var limit = parseInt(this.getParamValue('reviews_per_page', 1));
            var start = this.reviews().length;
            var end = _page*limit;
            if(layout=='carousel'){
                end = 9999;
            }
            for(var i=0;i<this.reviewsAll().length;i++){
                if(i<start || i>=end){
                    continue;
                }
                this.reviews.push(this.reviewsAll()[i]);
            }
            this.checkGrid();
        },
        ...
        clearAndLoad: function(){
            this.data({});
            this.currentPage = 1;
            this.reviewsAll.removeAll();
            this.reviews.removeAll();
            if(this.getConfigValue('mode','')=='frontend' || this.getParamValue('main_source_id',0)!=0){
                this.loadData();
            }
        },
        ...
        _loadData: function(_ajax_config){
            ...
            var ajaxConfig = {
                url: this.getConfigValue('ajax_url')+'&id='+this.getConfigValue('id')+requestAddStr,
                method: "GET",
                dataType: 'json',
                success: function(_data){
                    feed.dataLoading(false);
                    if(_data && _data.status && _data.status=='success'){
                        if(_data.params){
                            feed.addParams(_data.params);
                        }
                        feed.data(_data.data);
                        feed.reviewsAll(_data.reviews);
                        feed.showReviewsPage(1);
                    }
...
a4jRequire(["fpr_app", "fpr_widget", "jquery"], function(App, Widget, $){
    $(document).ready(function(){
        var configNode = $('#facebookpagereviews-config');
        if(configNode.length==0){
            App.showMessage('No FPR config', 'error');
        }
        var config = JSON.parse(configNode.html());
        if(!config || typeof config.widgetConfig=='undefined'){
            App.showMessage('Wrong FPR config', 'error');
        }
        
        Widget.setTranslation(config.i18n);
        
        $(".facebookpagereviews-widget").each(function(){
            var node = $(this);
